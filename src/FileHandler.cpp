...
void FileHandler::addSecLog(const char* role, int eid, const char* result) {
	...
	timestamp[2] = '-';
	timestamp[3] = ((ltm.tm_mon + 1) / 10) + '0';
	timestamp[4] = ((ltm.tm_mon + 1) % 10) + '0';
	timestamp[5] = '-';
	timestamp[6] = ((ltm.tm_year + 1900) / 1000) + '0';
	timestamp[7] = (((ltm.tm_year + 1900) % 1000) / 100) + '0';
	timestamp[8] = (((ltm.tm_year + 1900) % 100) / 10) + '0';
	timestamp[9] = ((ltm.tm_year + 1900) % 10) + '0';
	timestamp[10] = ' ';
	timestamp[11] = (ltm.tm_hour / 10) + '0';
	timestamp[12] = (ltm.tm_hour % 10) + '0';
	timestamp[13] = ':';
	timestamp[14] = (ltm.tm_min / 10) + '0';
	timestamp[15] = (ltm.tm_min % 10) + '0';
	timestamp[16] = '\0';

	f << timestamp << "," << role << "," << eid << "," << result << endl;
	f.close();
}
